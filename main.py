...
def passGen(number):
    newOne = random.sample(words, number)
    print("Initial password: ", end='')
    return "".join(newOne)


def rep_with_upper(stringToUpperCase):
    randomNumber = random.randint(0, len(stringToUpperCase))
    modified = list(stringToUpperCase)
    print(modified[randomNumber], "at index", randomNumber, "will be upper")
    modified[randomNumber] = modified[randomNumber].upper()
    newString = "".join(modified)
    return newString


def swap_letters(newModified):
    newList = list(newModified)
    newList[0], newList[-2] = newList[-2], newList[0]
    newList[1], newList[-1] = newList[-1], newList[1]
    newString = "".join(newList)
    print("Final version of the password: ", end='')
    return newString


def search_letter(stringToSearch, character):
    newList = list(stringToSearch)
    for i in newList:
        if i == character:
            return True
    return False
...
while True:
    ...
    if 7 < user_number or user_number < 3:
        ...
        print(password)
        upperCasedPassword = rep_with_upper(password)
        swappedPassword = swap_letters(upperCasedPassword)
        break

print(swappedPassword)
print(search_letter(swappedPassword, "d"))